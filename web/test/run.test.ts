...
import { PAYMENTS } from '../lib/demo/params';
...
function fakeDeps(): Deps & { calls: string[] } {
  ...
  return {
    ...
    pay: async () => {
      calls.push('pay');
      paid += 1;
      if (paid > PAYMENTS) throw new AllowanceRefused(106, 'Error(Contract, #106)');
...
test('the run resolves every step, in order, with both refusals where the story puts them', async () => {
  ...
  assert.deepEqual(done.map((e) => `${e.id}:${e.state}`), [
    ...
    'pay-1:done',
    'stranger:refused',
    ...Array.from({ length: PAYMENTS - 1 }, (_, i) => `pay-${i + 2}:done`),
    `pay-${PAYMENTS + 1}:refused`,
  ]);
  assert.equal(done.find((e) => e.id === 'stranger')?.rule, 'allowlist');
  assert.equal(done.find((e) => e.id === `pay-${PAYMENTS + 1}`)?.rule, 'window');
  assert.deepEqual(deps.calls, [
    ...
    'pay',
    'refuse',
    ...Array.from({ length: PAYMENTS }, () => 'pay'),
  ]);
});
